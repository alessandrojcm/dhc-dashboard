...
- Public workshop registration route contract: `+page.server.ts` should return `404` for unavailable workshops (`NOT_FOUND`/`NOT_PUBLISHED`/`NOT_PUBLIC`/missing external price), return `200` with only a generic "This workshop is full" message when full (no workshop payload), and completion remote responses should return `{ success: true, redirectTo: "/workshops/[id]/confirmation" }`.
- Public workshop registration backend (Stage 3 complete): `src/routes/(public)/workshops/[id]/register/+page.server.ts` loads gate status via `getExternalRegistrationGate()` service method. Remote functions in `data.remote.ts` expose `createExternalPaymentIntent` and `completeExternalRegistration` commands, both validating input with Valibot schemas from `$lib/schemas/workshop-registration.ts` and enforcing route/body workshopId match. Success/error responses follow standard contract: `{ success: true, ... }` or `{ success: false, error: string, code?: string }`.
