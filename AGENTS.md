...
| Add Phoenix Inventory read model/API | `apps/phoenix/lib/dhc/inventory.ex` (context) + `apps/phoenix/lib/dhc/inventory/` (schemas: `Container`, `EquipmentCategory`, `InventoryItem`) + `apps/phoenix/lib/dhc_web/controllers/inventory_controller.ex` | NEW — Inventory read-model helpers. Schemas map `inventory_items`/`equipment_categories`/`containers` persistence vocab; context/controllers return Inventory-vocabulary DTOs. `GET /api/inventory/overview` (issue ALE-94) returns the four overview counts and is management-only (`quartermaster`/`president`/`admin` via the `:inventory_admin_api` pipeline). Inventory Activity (the `inventory_history` feed) is a separate endpoint (PRD #93, ALE-95). The `equipment_categories.available_attributes` JSONB column stores a JSON array of attribute definitions; the schema types it `{:array, :map}` (not `:map`, which rejects arrays). |
