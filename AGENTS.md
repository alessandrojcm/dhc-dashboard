...
| Add Phoenix Inventory read model/API | `apps/phoenix/lib/dhc/inventory.ex` (context) + `apps/phoenix/lib/dhc/inventory/` (schemas: `Container`, `EquipmentCategory`, `InventoryItem`, `InventoryActivity`) + `apps/phoenix/lib/dhc_web/controllers/inventory_controller.ex` | NEW — Inventory read-model helpers. Schemas map `inventory_items`/`equipment_categories`/`containers`/`inventory_history` persistence vocab; context/controllers return Inventory-vocabulary DTOs. `GET /api/inventory/overview` (issue ALE-94) returns the four overview counts and is management-only (`quartermaster`/`president`/`admin` via the `:inventory_admin_api` pipeline). `GET /api/inventory/activity` (issue ALE-95) is the cursor-paginated newest-first Inventory Activity feed (no total count; supports `itemId`/`containerId` filters; cursor pattern mirrors Notifications — forward-only, opaque base64url JSON binding `{limit, itemId, containerId, id, createdAt}`). `GET /api/inventory/items/filters` (issue ALE-98) returns the Equipment Category + Container dropdown options for the item list (`{ data: { categories, containers } }`; each category: `id`, `name`, `description`, `availableAttributes`, `attributeSchema`; each container: `id`, `name`, `description`, `parentContainerId`; both ordered by `name` asc; internal timestamps and `created_by` are not exposed). `GET /api/inventory/categories` (issue ALE-96) returns the domain-shaped Equipment Category list with item counts (`{ data: { categories } }`; each category: `id`, `name`, `description`, `availableAttributes`, `itemCount`; ordered by `name` asc; item count is a left-joined `count(inventory_items.id)` aggregate so empty categories still appear with `itemCount: 0`; internal timestamps and `created_by` are not exposed). `GET /api/inventory/containers` (issue ALE-97) returns the flat Container list with parent and item counts (`{ data: { containers } }`; each container: `id`, `name`, `description`, `parentContainerId`, `parentContainer` (the nested `{ id, name }` object, or `null` for a top-level Container), `itemCount`; ordered by `name` asc; item count is a left-joined `count(inventory_items.id)` aggregate so empty containers still appear with `itemCount: 0`; the response is a flat array — the dashboard derives its hierarchy tree client-side; the `parentContainer` object is built in Elixir from the same flat list rather than via a self-join, so a missing parent stays `nil`; internal timestamps and `created_by` are not exposed). `GET /api/inventory/items` (issue ALE-99) is the cursor-paginated Inventory Item list with `totalCount` (`{ data: { items, limit, totalCount, nextCursor, previousCursor } }`; each item: `id`, `quantity`, `maintenanceStatus` (`available`/`inMaintenance` — the domain term for the persistence `out_for_maintenance` boolean), `attributes`, `category` (`{ id, name }` or `null`), `container` (`{ id, name }` or `null`); fixed sort `createdAt desc, id desc` — no caller-selected sort in this slice; `totalCount` is an exact `COUNT(*)` matching the active filters; bidirectional cursor pagination mirrors the Members/Waitlist table — opaque base64url JSON binding `{limit, q, categoryId, containerId, maintenanceStatus, id, createdAt, pageDirection}`; `q` ilike searches `attributes->>name`, Equipment Category name, Container name; `categoryId`/`containerId`/`maintenanceStatus=all|inMaintenance|available` filters; internal auth-user refs and timestamps are not exposed). The `equipment_categories.available_attributes` JSONB column stores a JSON array of attribute definitions; the schema types it `{:array, :map}` (not `:map`, which rejects arrays). `inventory_history` uses a standalone `created_at` field (not `timestamps/1` `inserted_at`) — the `InventoryActivity` schema declares `field :created_at, :utc_datetime` directly, mirroring production. |
...
| Generate controllers from spec | Run `mise run gen-controllers` (or `cd apps/phoenix && mix gen.controllers` only if mise is unavailable) | Generates controller + JSON renderer + contract test per tag. REST mapping from HTTP method + path. `--force` overwrites all, `--force=<path>` overwrites specific file. Always prefer mise so `.env` loads. |
...
- **Inventory read API design**: Inventory PostgREST/server-read migration uses domain terms Inventory Item, Equipment Category, Container, and Inventory Activity. Initial read RBAC mirrors the current dashboard gate (`quartermaster`, `admin`, `president`). Preserve existing read payload fields/features unless changing for established Phoenix API consistency (for example cursor pagination), domain-model clarity, or more RESTful URI design. **Read migration complete (ALE-93 umbrella: ALE-94 overview, ALE-95 activity, ALE-96 categories, ALE-97 containers, ALE-98 item filters, ALE-99 item list, ALE-100 cleanup).** All six Inventory dashboard reads flow through `@dhc/api-client` Phoenix APIs (browser-direct TanStack Query); no `supabase.from(...)`/Kysely/SvelteKit server-loader DB reads remain in the list/overview paths. Single-record reads (detail/edit pages) and writes (`data.remote.ts`) still use the SvelteKit service layer (out of scope). See `docs/agents/postgrest-read-migration.md` for the full inventory.
...
- **PostgREST read migration**: Replace SvelteKit `supabase.from(...).select(...)` reads with domain-shaped Phoenix APIs, not table/view proxies. Track remaining call sites in `docs/agents/postgrest-read-migration.md`; first slice is Waitlist per ADR 0005. **Cleanup-slice pattern**: after migrating list reads to Phoenix, check for orphaned SvelteKit service-layer list read methods (`findMany`/`getRecent`/etc.) that were only called by the old `+page.server.ts` reads — they become dead code and should be removed in the cleanup slice along with their now-unused types. Single-record reads (`findById`/`getWithRelations`) and writes stay on the service layer (out of scope for read migrations).
