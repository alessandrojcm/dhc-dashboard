defmodule DhcWeb.InventoryItemsControllerTest do
  ...
  describe "show" do
    test "returns the viewer shape with derived label, values, and availability", %{conn: conn} do
      ...
      conn =
        conn |> auth_conn("quartermaster") |> get("/api/inventory/items/#{item.slug}")
    ...
    test "resolves by slug or id and 404s otherwise", %{conn: conn} do
      ...
      missing =
        build_conn()
        |> auth_conn("quartermaster")
        |> get("/api/inventory/items/item-999999")
    ...
    test "grants every operator role equal read authority and 401s without a session", %{
      ...
      for role <- @write_roles do
        read =
          build_conn() |> auth_conn(role) |> get("/api/inventory/items/#{item.slug}")

        assert json_response(read, 200)["data"]["slug"] == item.slug
      end

      anonymous = get(conn, "/api/inventory/items/#{item.slug}")
    ...
    test "refuses members, who must not see containers or operator facts", %{conn: conn} do
      ...
      show = conn |> auth_conn("member") |> get("/api/inventory/items/#{item.slug}")
      assert %{"errors" => %{"detail" => "Insufficient role"}} = json_response(show, 403)

      list = build_conn() |> auth_conn("member") |> get("/api/inventory/items")
  ...
  describe "update" do
    ...
    test "409s an archived item as a domain conflict", %{conn: conn} do
      ...
      conn =
        conn
        |> auth_conn("admin")
        |> patch("/api/inventory/items/#{item.slug}", %{"notes" => "nope"})
  ...
  describe "category" do
    test "reclassifies atomically with the new category's values", %{conn: conn} do
      ...
      conn =
        conn
        |> auth_conn("admin")
        |> post("/api/inventory/items/#{item.slug}/category", %{
  ...
  describe "archive and restore" do
    test "archives with a reason then restores", %{conn: conn} do
      ...
      archived =
        conn
        |> auth_conn("quartermaster")
        |> post("/api/inventory/items/#{item.slug}/archive", %{"reason" => "Retired"})
  ...
  describe "delete" do
    test "deletes a history-free item with confirmation", %{conn: conn} do
      ...
      conn =
        conn
        |> auth_conn("admin")
        |> delete("/api/inventory/items/#{item.slug}", %{"confirm" => true})
  ...
  describe "openapi operator item contract" do
    ...
    test "the list operation declares cursor pagination with exact counts" do
      ...
      params =
        get_in(spec, ["paths", "/inventory/items", "get", "parameters"])
        |> Map.new(&{&1["name"], &1})
  ...
  defp operator_item_operations do
    [
      {"/inventory/items", "get", "inventoryItems.list"},
      {"/inventory/items", "post", "inventoryItems.create"},
      {"/inventory/items/{slugOrId}", "get", "inventoryItems.show"},
      {"/inventory/items/{slugOrId}", "patch", "inventoryItems.update"},
      {"/inventory/items/{slugOrId}", "delete", "inventoryItems.delete"},
      {"/inventory/items/{slugOrId}/category", "post", "inventoryItems.changeCategory"},
      {"/inventory/items/{slugOrId}/move", "post", "inventoryItems.move"},
      {"/inventory/items/{slugOrId}/maintenance", "get", "inventoryItems.listMaintenance"},
      {"/inventory/items/{slugOrId}/maintenance/start", "post",
       "inventoryItems.startMaintenance"},
      {"/inventory/items/{slugOrId}/maintenance/end", "post", "inventoryItems.endMaintenance"},
      {"/inventory/items/{slugOrId}/archive", "post", "inventoryItems.archive"},
      {"/inventory/items/{slugOrId}/restore", "post", "inventoryItems.restore"}
    ]
  end
