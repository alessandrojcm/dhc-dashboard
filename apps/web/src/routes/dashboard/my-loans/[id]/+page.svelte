...
const cancelMutation = createMutation(() => ({
	...
	onSuccess: () => {
		cancelNote = "";
		queryClient.invalidateQueries({ queryKey: ["inventoryMemberLoansShow"] });
		queryClient.invalidateQueries({ queryKey: ["inventoryMemberLoansList"] });
...
<div class="mx-auto max-w-md px-4 pt-5 pb-10 sm:px-5">
	...
	{:else if loanQuery.data}
		...
		<section aria-labelledby="loan-heading" class="space-y-4">
			<div>
				<div class="flex items-start justify-between gap-2">
					<h1 id="loan-heading" class="font-heading text-2xl font-bold">
						{loan.itemLabel}
					</h1>
			...
			{:else if loan.status === "checked_out"}
				...
				>
					This item is with you — hand it back to a quartermaster to complete
					the return.
				</p>
			{/if}
		</section>
