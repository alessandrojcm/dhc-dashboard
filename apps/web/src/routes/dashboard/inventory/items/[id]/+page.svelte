...
<div class="p-6 pb-24">
	...
	<form {...updateItem}>
		...
		<input
			type="hidden"
			name="category_id"
			value={data.initialFormData.category_id}
		/>
					...
					<CardContent class="space-y-4">
						{#if isEditMode}
							<div class="grid gap-4 md:grid-cols-2">
								...
								<div>
									...
									<div
										class="flex items-center gap-2 mt-1 p-3 bg-muted rounded-md"
									>
										<Tags class="h-4 w-4 text-muted-foreground" />
										<span class="text-sm"
											>{displayItem.category?.name || "Uncategorized"}</span
										>
									</div>
									...
									<div class="flex items-center space-x-2 pt-6">
										<Checkbox
											...
											onCheckedChange={(checked) => {
												...
												updateItem.fields.out_for_maintenance.set(
													outForMaintenance,
												);
											}}
										/>
							...
							<div class="grid gap-4 md:grid-cols-2">
								...
								<div>
									...
									<div class="flex items-center gap-2 mt-1">
										...
										<Button
											href="/dashboard/inventory/containers/{displayItem
												.container?.id ?? ''}"
											variant="link"
						...
						<div class="grid gap-4 md:grid-cols-2">
							<div>
								...
								<p class="text-sm text-muted-foreground mt-1">
									{dayjs(displayItem.created_at).format(
							...
							<div>
								...
								<p class="text-sm text-muted-foreground mt-1">
									{dayjs(displayItem.updated_at).format(
										"MMM D, YYYY [at] h:mm A",
									)}
								</p>
							</div>
						...
						<CardContent class="space-y-3">
							{#if canEdit}
								...
								<Button
									...
									variant={displayItem.out_for_maintenance
										? "default"
										: "outline"}
									class="w-full"
								...
								>
									...
									{maintenanceMutation.isPending
										? "Updating..."
										: displayItem.out_for_maintenance
											? "Mark Available"
											: "Mark Out for Maintenance"}
								</Button>
							{/if}
							<Button
								href="/dashboard/inventory/containers/{displayItem.container
									?.id ?? ''}"
								variant="outline"
								class="w-full"
							>
									...
									>
										<SelectTrigger>
											{hierarchicalContainers.find(
												(c) => c.id === moveTargetContainerId,
											)?.displayName || "Select a container"}
										</SelectTrigger>
										<SelectContent>
											{#each hierarchicalContainers as container (container.id)}
												<SelectItem value={container.id}>
							...
							<div class="space-y-3">
								{#each currentHistory as entry (entry.id)}
									...
									<div class="flex items-start gap-3">
										...
										>
											<ActionIcon
												class="h-4 w-4 {getActionColor(entry.action)}"
											/>
										</div>
										<div class="flex-1 space-y-1">
											<p class="text-sm">
												<span class="font-medium"
													>{formatActionLabel(entry.action)}</span
												>
												{#if entry.action === "moved" && entry.old_container && entry.new_container}
													from {entry.old_container.name} to {entry
														.new_container.name}
												{/if}
											</p>
											{#if entry.notes}
		...
		{#if isEditMode}
			...
			>
				<div
					class="container flex h-16 items-center justify-between max-w-7xl mx-auto px-6"
				>
					<p class="text-sm text-muted-foreground">
						Make changes to the item details above
					</p>
