...
function descendantsOf(id: string): Set<string> {
	...
	const visit = (parent: string) => {
		for (const child of containersQuery.data ?? [])
...
<svelte:head><title>Inventory containers | Dublin HEMA Club</title></svelte:head
>
<div class="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6">
	...
	<div class="grid gap-6 lg:grid-cols-[22rem_1fr]">
		<form class="space-y-4 rounded-2xl border bg-card p-5" onsubmit={submit}>
			...
			<div>
				<Label for="container-name">Name</Label><Input
					id="container-name"
					maxlength={100}
					required
					bind:value={name}
			...
			<div>
				<Label for="container-description">Description</Label><Input
			...
			<div>
				<Label for="container-parent">Parent</Label><select
					id="container-parent"
					class="h-10 w-full rounded-md border bg-background px-3"
					bind:value={parentId}
					><option value="">Root</option
					>{#each containersQuery.data ?? [] as candidate (candidate.id)}{@const excluded =
							editing
								? descendantsOf(editing.id)
								: new Set<string>()}{#if !candidate.archivedAt && candidate.id !== editing?.id && !excluded.has(candidate.id)}<option
			...
			<div class="flex gap-2">
				<Button type="submit"
					>{editing ? "Save changes" : "Add container"}</Button
				>{#if editing}<Button type="button" variant="outline" onclick={reset}
						>Cancel</Button
					>{/if}
			</div>
		</form>
		<section class="space-y-3">
			...
			{#each containersQuery.data ?? [] as container (container.id)}<article
					...
					<div class="flex flex-wrap items-start justify-between gap-3">
						<div class="flex gap-3">
							...
							<div>
								<div class="flex flex-wrap items-center gap-2">
									...
									{#if container.archivedAt}<Badge variant="outline"
											>Archived</Badge
										>{/if}
								</div>
								...
								{#if container.description}<p class="mt-1 text-sm">
						...
						<div class="flex flex-wrap gap-2">
							<Button
								size="sm"
								variant="outline"
								onclick={() => startEdit(container)}>Edit / move</Button
							>{#if container.archivedAt}<Button
									size="sm"
									variant="outline"
									onclick={() =>
