	...
	<div class="max-w-2xl">
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Update the container information. Be careful when changing the parent
					container as it affects the hierarchy.
				</CardDescription>
			</CardHeader>
			<CardContent>
				<form {...updateContainer} class="space-y-6">
					<Field.Field>
						{@const fieldProps = updateContainer.fields.name.as("text")}
					...
					<Field.Field>
						{@const fieldProps = updateContainer.fields.description.as("text")}
					...
					<Field.Field>
						...
						<Select
							...
							onValueChange={(v) =>
								updateContainer.fields.parent_container_id.set(v)}
						...
						<Field.Description>
							Choose a parent container to create a hierarchy. Leave empty to
					...
					<div class="flex gap-3 pt-4">
						<Button type="submit" disabled={!!updateContainer.pending}>
							{updateContainer.pending ? "Updating..." : "Update Container"}
						</Button>
						<Button
							href="/dashboard/inventory/containers/{data.container.id}"
							variant="outline"
				...
				{:else}
					<div class="space-y-4">
						<p class="text-sm text-muted-foreground">
							...
							delete all child containers and move any items to the parent
							container or root level.
						</p>
