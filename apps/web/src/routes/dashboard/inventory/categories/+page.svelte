	...
	<div class="flex items-center justify-between mb-6">
		<div>
			...
			<p class="text-muted-foreground">
				Manage equipment types and their attributes
			</p>
		</div>
	...
	{:else if categoriesQuery.isError}
		<Card>
			<CardContent class="flex flex-col items-center justify-center py-12">
				...
				<Button onclick={() => categoriesQuery.refetch()} variant="outline"
					>Retry</Button
				>
			</CardContent>
		</Card>
	{:else if categoriesQuery.data.length === 0}
		<Card>
			<CardContent class="flex flex-col items-center justify-center py-12">
				...
				<p class="text-muted-foreground mb-4">
					Create your first equipment category to start organizing your
					inventory
				</p>
				<Button href="/dashboard/inventory/categories/create">
					<Plus class="mr-2 h-4 w-4" />
						...
						<div class="flex items-center gap-4 mb-4">
							<div class="flex items-center gap-1">
								<Badge variant="secondary" class="text-xs">
									{getAttributeCount(category)} attribute{getAttributeCount(
										category,
									) !== 1
										? "s"
										: ""}
							...
							<div class="flex items-center gap-1">
								...
								>
									...
									{getItemCount(category)} item{getItemCount(category) !== 1
										? "s"
										: ""}
								</Badge>
							</div>
						...
						{#if getAttributeCount(category) > 0}
							<div class="space-y-2">
								...
								<div class="flex flex-wrap gap-1">
									{#each category.availableAttributes as attr (attr.name ?? attr.label)}
