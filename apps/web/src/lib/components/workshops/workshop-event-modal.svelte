...
<div class="workshop-event-modal">
		...
		<div class="flex items-start justify-between">
			<div class="space-y-2">
				<h2 class="text-xl font-semibold text-foreground leading-tight">
					{workshop?.title || event?.title || "Workshop"}
				</h2>
				<div class="flex items-center gap-2">
					<Badge
						variant={workshop.status === "planned"
							? "secondary"
	...
	<div class="modal-content px-6 pb-6 space-y-5">
		...
		<div class="flex items-start gap-3">
			...
			<div class="space-y-1">
				<div class="font-medium text-foreground">
					{dayjs(workshop?.startDate || event?.start).format(
						"dddd, MMM DD, YYYY",
					)}
				</div>
		...
		{#if workshop.isPublic && workshop.status === "published"}
			<div
				class="flex items-center justify-between gap-4 rounded-lg border bg-muted/30 p-3"
			>
				<div class="min-w-0">
					<p class="text-sm font-medium text-foreground">
						Share public workshop
					</p>
					<p class="text-xs text-muted-foreground">
						Copy the registration link to share it.
					</p>
				</div>
				<Button
					variant="outline"
		...
		<!-- Pricing -->
		<div
			class="bg-gradient-to-r from-muted/30 to-muted/20 p-4 rounded-lg border space-y-3"
		>
			<div class="flex items-center justify-between">
				<span class="text-sm font-medium text-foreground">Member Price:</span>
				<span class="text-lg font-bold text-foreground"
					>{formatPrice(workshop.priceMember ?? 0)}</span
				>
			</div>
			{#if workshop.isPublic}
				<div class="flex items-center justify-between">
			...
			<div class="flex flex-wrap gap-3">
				...
				{#if workshop.status === "published"}
					<Button
						variant="default"
						size="sm"
						href={`workshops/${workshop.id}/attendees`}
					>
						Manage attendees
					</Button>
				{/if}
