...
- Root container names are globally unique: the E2E database is shared per run, so every `inventoryStructure` seed needs a distinct root container path — never rely on the wrapper default across specs, or the second seed 422s on `containers_root_name_unique`.
