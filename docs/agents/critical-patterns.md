...
- **Lock order is part of the contract, not just lock presence.** Every availability-changing Inventory command locks the *item* `FOR UPDATE` first and only then the dependent row (loan, maintenance period). A command that locks the dependent row first — e.g. a member cancelling by loan id — deadlocks against an approval that holds the item and wants the loan. When a command is reached by a dependent row's id, resolve that row's `item_id` unlocked purely to learn *which* item to lock, take the item lock, then re-resolve ownership and status under it so nothing is decided from the unlocked read (`Dhc.Inventory.MemberLoans.lock_own_loan/2`).
- **A command reached by a dependent row's id still locks the item first.** Every operator loan transition in `Dhc.Inventory.OperatorLoans` is addressed by *loan* id but changes item availability, so it resolves `item_id` unlocked purely to learn which item to lock, takes the item `FOR UPDATE`, then re-reads the loan `FOR UPDATE` under it — the same shape as `MemberLoans.lock_own_loan/2`. Nothing is decided from the unlocked read.
...
- **`operationId` is `<slice>.<action>`**: the prefix must underscore to the controller filename that serves the operation (`inventoryOperatorItems.list` → `inventory_operator_items_controller.ex`). `mix gen.controllers` derives its files from this, so a mismatched prefix makes it emit an unwanted stub.
