...
test("underage user (16-17) should see guardian fields", async ({ page }) => {
	...
	await page
		.getByRole("option", { name: underageTestData.dateOfBirth.format("MMMM") })
		.dblclick();
	await page
		.getByLabel(underageTestData.dateOfBirth.format("dddd, MMMM D,"))
		.click();
...
test("adult user (18+) should not see guardian fields", async ({ page }) => {
	...
	await page.getByLabel("Select year").click();
	await page
		.getByRole("option", { name: adultTestData.dateOfBirth.year().toString() })
		.click();
	await page.getByLabel("Select month").click();
	await page
		.getByRole("option", { name: adultTestData.dateOfBirth.format("MMMM") })
		.dblclick();
	await page
		.getByLabel(adultTestData.dateOfBirth.format("dddd, MMMM D,"))
		.click();
...
test("underage user (16-17) should be required to fill guardian fields", async ({
	page,
}) => {
	// Navigate to the form page
	await page.goto("/waitlist");
...
test("underage user (16-17) should be able to submit with guardian information", async ({
	...
	await page.fill(
		'input[name="guardianFirstName"]',
		underageTestData.guardianFirstName,
	);
	await page.fill(
		'input[name="guardianLastName"]',
		underageTestData.guardianLastName,
	);

	// Find the guardian phone input field
	const guardianPhoneInputField = page.getByLabel("Guardian Phone Number");

	await guardianPhoneInputField.pressSequentially(
		underageTestData.guardianPhoneNumber,
		{
			delay: 50,
		},
	);
...
test("adult user (18+) should be able to submit without guardian information", async ({
	page,
}) => {
	// Navigate to the form page
	await page.goto("/waitlist");
	...
	await page
		.getByLabel(/any medical condition/i)
		.fill(adultTestData.medicalConditions);
