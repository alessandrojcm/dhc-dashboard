...
test.describe("Inventory Items Management", () => {
	...
	test.beforeAll(async () => {
		...
		const weaponsData = await createCategory(
			`Weapons ${timestamp}`,
			"Weapons category",
			[
				{
	...
	test.describe("Item CRUD Operations", () => {
		test("should create basic item", async ({ page, context }) => {
			...
			await expect(page).toHaveURL(
				/\/dashboard\/inventory\/items\/[a-f0-9-]+$/,
			);
		...
		test("should create item with complex attributes", async ({
			page,
			context,
		}) => {
			await loginAsUser(context, quartermasterData.email);
			...
			await page
				.getByLabel(/notes/i)
				.fill(`Notes for complex item ${timestamp}`);
	...
	test.describe("Item Search and Filtering", () => {
		...
		test("should filter items by container", async ({ page, context }) => {
			...
			// Initially both items should be visible
			await expect(
				page.getByText(`First Container Item ${timestamp}`),
			).toBeVisible();
			await expect(
				page.getByText(`Second Container Item ${timestamp}`),
			).toBeVisible();
		...
		test("should filter items by maintenance status", async ({
			page,
			context,
		}) => {
			await loginAsUser(context, quartermasterData.email);
			...
			await expect(
				page.getByText(`Maintenance Item ${timestamp}`),
			).toBeVisible();
			...
			await expect(
				page.getByText(`Available Item ${timestamp}`),
			).not.toBeVisible();
		...
		test("should clear all filters when clear button is clicked", async ({
			...
			await expect(
				page.getByText(`Another Item ${timestamp}`),
			).not.toBeVisible();
		...
		test("should apply multiple filters simultaneously", async ({
			...
			await expect(
				page.getByText(`Multi Other ${timestamp}`),
			).not.toBeVisible();
			await expect(
				page.getByText(`Multi Maintenance Weapon ${timestamp}`),
			).not.toBeVisible();
		});
		...
		test("should persist filters across page navigation", async ({
			...
			await expect(
				page.getByText(`Persistent Item ${timestamp}`),
			).toBeVisible();
		});
	...
	test.describe("Access Control", () => {
		...
		test("should show different actions for different roles", async ({
			page,
			context,
		}) => {
			...
			await expect(
				page.getByRole("link", { name: /edit item/i }),
			).toBeVisible();
			await expect(
				page.getByRole("link", { name: /view container/i }),
			).toBeVisible();
	...
	test.describe("Item History and Audit Trail", () => {
		test("should display item history on detail page", async ({
			page,
			context,
		}) => {
			await loginAsUser(context, quartermasterData.email);
			...
			await page
				.getByLabel(/notes/i)
				.fill(`Item for history testing ${itemName}`);
			await page.getByLabel(/quantity/i).fill("1");
			await page.getByRole("button", { name: /category/i }).click();
			await page.getByText(/^Test Category/).click();
			await page.getByRole("button", { name: /container/i }).click();
