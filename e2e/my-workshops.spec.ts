...
test.describe("My Workshops Page", () => {
	...
	test.beforeEach(async () => {
		...
		if (plannedError) {
			throw new Error(
				`Failed to create planned workshop: ${plannedError.message}`,
			);
		}
		...
		if (publishedError) {
			throw new Error(
				`Failed to create published workshop: ${publishedError.message}`,
			);
		}
	...
	test("should load my-workshops page with correct structure", async ({
		...
		await expect(
			page.locator('button[data-value="published"]:has-text("Upcoming")'),
		).toBeVisible();
		await expect(
			page.locator('button[data-value="planned"]:has-text("Planned")'),
		).toBeVisible();

		// Check that Upcoming tab is active by default
		await expect(
			page.locator('button[data-value="published"][data-state="active"]'),
		).toBeVisible();
	});

	test("should display published workshops in upcoming tab", async ({
		page,
		context,
	}) => {
		await loginAsUser(context, memberData.email);
		...
		await expect(
			page.locator(`text=Published Workshop ${timestamp}`),
		).toBeVisible();
		await expect(
			page.locator("text=Test published workshop for my-workshops page"),
		).toBeVisible();
	...
	test("should display planned workshops in planned tab", async ({
		page,
		context,
	}) => {
		await loginAsUser(context, memberData.email);
		...
		await expect(
			page.locator('button[data-value="planned"][data-state="active"]'),
		).toBeVisible();

		// Check that planned workshop is displayed
		await expect(
			page.locator(`text=Planned Workshop ${timestamp}`),
		).toBeVisible();
		await expect(
			page.locator("text=Test planned workshop for my-workshops page"),
		).toBeVisible();
	...
	test("should allow member to express interest in planned workshop", async ({
		page,
		context,
	}) => {
		await loginAsUser(context, memberData.email);
		...
		await expect(
			page.locator('button:has-text("Withdraw Interest")'),
		).toBeVisible();
	...
	test("should not show interest buttons for published workshops", async ({
		page,
		context,
	}) => {
		await loginAsUser(context, memberData.email);
	...
	test("should show loading state when switching tabs", async ({
		page,
		context,
	}) => {
		await loginAsUser(context, memberData.email);
		...
		const loadingOrContent = page.locator(
			'[data-testid="loading-skeleton"], .space-y-4',
		);
	...
	test("should handle empty state when no workshops exist", async ({
		page,
		context,
	}) => {
	...
	test("should display workshop details correctly", async ({
		page,
		context,
	}) => {
		await loginAsUser(context, memberData.email);
	...
	test("should maintain tab state when navigating back", async ({
		page,
		context,
	}) => {
		await loginAsUser(context, memberData.email);
	...
	test("should handle interest toggle loading state", async ({
		page,
		context,
	}) => {
		await loginAsUser(context, memberData.email);
	...
	test("should display status badges with correct colors", async ({
		...
		await expect(
			page.locator('.bg-green-500:has-text("published")'),
		).toBeVisible();
		...
		// Check planned badge
		await expect(
			page.locator('.bg-yellow-500:has-text("planned")'),
		).toBeVisible();
	});
