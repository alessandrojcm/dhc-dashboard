...
test.describe("Inventory Containers Management", () => {
	...
	test.describe("Container CRUD Operations", () => {
		test("should create root container as quartermaster", async ({
			page,
			context,
		}) => {
			...
			await page
				...
				.fill("Test room for equipment storage");
			// Submit form
		...
		test("should create nested container as quartermaster", async ({
			page,
			context,
		}) => {
			await loginAsUser(context, quartermasterData.email);
		...
		test("should edit container as quartermaster", async ({
			page,
			context,
		}) => {
			await loginAsUser(context, quartermasterData.email);
			...
			await page
				.getByRole("link", { name: `Edit ${originalName?.name}` })
				.click();
		...
		test("should delete empty container as quartermaster", async ({
			page,
			context,
		}) => {
			await loginAsUser(context, quartermasterData.email);
			...
			// Confirm deletion
			await page
				.getByRole("button", { name: /yes, delete container/i })
				.click();
		...
		test("should prevent deletion of container with items", async ({
			page,
			context,
		}) => {
			await loginAsUser(context, quartermasterData.email);
			...
			await expect(
				page.getByText(/cannot delete a container that contains items/i),
			).toBeVisible();
		});
	});

	test.describe("Access Control", () => {
		test("should allow quartermaster full access to containers", async ({
			page,
			context,
		}) => {
			await loginAsUser(context, quartermasterData.email);
			...
			await expect(
				page.getByRole("link", { name: "Add Container" }).first(),
			).toBeVisible();
		...
		test("should allow members read-only access to containers", async ({
			page,
			context,
		}) => {
			...
			await expect(
				page.getByRole("link", { name: "Add Container" }),
			).not.toBeVisible();
		});

		test("should allow admin full access to containers", async ({
			page,
			context,
		}) => {
	...
	test.describe("Container Hierarchy Display", () => {
		test("should display container hierarchy correctly", async ({
			page,
			context,
		}) => {
			await loginAsUser(context, quartermasterData.email);
			...
			const [c1, c2, c3] = await createContainer()
				...
				.then(async (c) => [
					...c,
					await createContainer({ parentId: c[1]?.id }),
				]);
			...
			await expect(
				page.getByRole("heading", { name: c1?.name }).first(),
			).toBeVisible();
			await expect(
				page.getByRole("heading", { name: c2?.name }).first(),
			).toBeVisible();
			await expect(
				page.getByRole("heading", { name: c3?.name }).first(),
			).toBeVisible();

			// Should show proper nesting indicators
			await expect(
				page.locator('[data-testid="container-hierarchy"]'),
			).toBeVisible();
		});
	});
