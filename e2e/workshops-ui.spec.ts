...
test.describe("Workshop UI", () => {
	...
	test("should display and validate workshop creation form", async ({
		...
		await expect(
			page.getByRole("textbox", { name: /workshop time/i }),
		).toBeVisible();
		await expect(
	...
	test("should successfully create a workshop through UI", async ({
		...
		await page
			.getByRole("option", { name: workshopDate.year().toString() })
			.click();
		await page.getByRole("button", { name: "Select month" }).click();
		await page
			.getByRole("option", { name: workshopDate.format("MMMM") })
			.click();
		await page
			.getByRole("button", { name: workshopDate.format("dddd, MMMM D") })
			.click();
		await page
