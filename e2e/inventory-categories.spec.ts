...
async function makeAuthenticatedRequest(
	...
	void options;
	throw new Error(
		"Inventory category E2E tests are pending Phoenix helper migration",
	);
}

test.describe
	.skip("Inventory Categories Management", () => {
		...
		test.describe("Category CRUD Operations", () => {
			test("should create basic category as quartermaster", async ({
				...
				await page
					...
					.fill("Test category for equipment");
			...
			test("should delete category with no items", async ({
				page,
				context,
			}) => {
				await loginAsUser(context, quartermasterData.email);
		...
		test.describe("Attribute Builder Functionality", () => {
			...
			test("should handle attribute validation errors", async ({
				...
				await expect(
					page.getByText(
						/invalid type: expected string but received undefined/i,
					),
				).toBeVisible();
			});
		...
		test.describe("Category API Endpoints", () => {
			...
			test("should update category via API", async ({ page, context }) => {
				...
				expect(
					updateResponse.category.available_attributes.model,
				).toBeDefined();
			});
