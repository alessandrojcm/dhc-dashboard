...
test.describe("Attendee Management UI", () => {
	...
	test("should display refund management section", async ({
		page,
		context,
	}) => {
		await loginAsUser(context, adminData.email);
		...
		await expect(page.getByText("Refund Management")).toBeVisible();
		await expect(
			page.getByRole("button", { name: "Process Refund" }),
		).toBeVisible();
	});
	...
	test("should open refund dialog when process refund clicked", async ({
		...
		await expect(
			page.getByRole("heading", { name: "Process Refund" }),
		).toBeVisible();
		...
		await expect(
			page.getByPlaceholder("Enter reason for refund..."),
		).toBeVisible();
	...
	test("should process refund through UI", async ({ page, context }) => {
		...
		// Enter refund reason
		await page
			.getByPlaceholder("Enter reason for refund...")
			.fill("Test refund through UI");

		// Submit refund - find the Process Refund button inside the dialog
		await page
			.getByRole("dialog")
			.getByRole("button", { name: "Process Refund" })
			.click();
	...
	test("should display existing refunds", async ({ page, context }) => {
		...
		const existingRefundsSection = page
			.locator("div")
			.filter({ hasText: "Existing Refunds" });
		await expect(
			existingRefundsSection.locator('[data-slot="badge"]').first(),
		).toBeVisible();
	});

	test("should show no refunds message when none exist", async ({
		page,
		context,
	}) => {
		await loginAsUser(context, adminData.email);
	...
	test("should validate refund form inputs", async ({ page, context }) => {
		...
		await expect(
			page.getByText("Please select an attendee and provide a reason"),
		).toBeVisible({
			timeout: 5000,
		});
	});
