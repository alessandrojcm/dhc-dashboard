...
test.describe("Inventory Management Full Lifecycle", () => {
	...
	test("should complete full inventory setup and management workflow", async ({
		...
		await expect(
			page.getByRole("heading", { name: /main storage/i }),
		).toBeVisible();
		...
		await page.getByRole("button", { name: /create/i }).click();
		await expect(
			page.getByRole("heading", { name: /weapon rack/i }),
		).toBeVisible();
		...
		await page
			...
			.fill("Cabinet for protective equipment");
		await page.getByLabel(/parent container/i).click();
		await page.getByText(`Main Storage ${timestamp}`).click();
		await page.getByRole("button", { name: /create/i }).click();
		await expect(
			page.getByRole("heading", { name: /Armor cabinet/i }),
		).toBeVisible();
		...
		await page
			.getByLabel(/notes/i)
			.fill("High-quality synthetic training longsword");
		...
		await page
			.getByLabel(/notes/i)
			.fill("Standard fencing masks for protection");
		...
		await expect(
			page.getByText(`Fencing Masks ${timestamp}`),
		).not.toBeVisible();
		...
		await expect(
			page.getByRole("link", { name: /add container/i }),
		).not.toBeVisible();
	...
	test("should handle inventory data validation correctly", async ({
		page,
		context,
	}) => {
		await loginAsUser(context, quartermasterData.email);
