...
test.describe("Invitations Manager", () => {
	...
	test("should be able to resend an expired invitation link", async ({
		page,
	}) => {
		// Navigate to members page
		await page.goto("/dashboard/members");
	...
	test("should handle errors when resending invitation link", async ({
		page,
	}) => {
		// Navigate to members page
		await page.goto("/dashboard/members");
	...
	test("should sort invitations by different columns", async ({ page }) => {
		...
		await expect(
			page.locator("th").filter({ hasText: "Email" }).locator("svg"),
		).toBeVisible();
		...
		// Check if sorting indicator is visible
		await expect(
			page.locator("th").filter({ hasText: "Created" }).locator("svg"),
		).toBeVisible();
	});
});
