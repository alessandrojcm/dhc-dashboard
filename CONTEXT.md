...
| **Staged Discord Assignment** | A reviewed, pre-use mapping from an existing Member's Authentication Principal to one Discord provider subject selected from a restricted one-off guild-roster export. It is not an External Identity or login capability: only its first matching Discord OAuth proof atomically promotes it to the External Identity. User ID is the binding fact; the stored username snapshot is review metadata only. |
