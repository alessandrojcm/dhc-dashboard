...
| **Inventory** | Club-owned physical equipment. Each Item is one physical unit assigned to one Category and stored directly in one Container. Routine edits and movements are current facts rather than retained audit history. |
| **Maintenance Period** | An interval during which an Item is unavailable for requests and Loans. Its start and end retain when the change occurred and the Inventory Operator who recorded it; the system does not separately model the person who physically performed the work. |
...
| **Acceptance Session** | One browser's Invitation Acceptance journey as owned by `Dhc.Onboarding.Acceptance`, identified only by an opaque handle held in a signed HttpOnly cookie. It spans the Continuation, Attempt, Discord Subject Claim, and payment lease without exposing which of them the workflow currently rests in; every browser-facing operation returns a discriminated safe view. |
