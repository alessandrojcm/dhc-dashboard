...
| **Inventory** | Club equipment (swords, masks, etc.) tracked as Inventory Items in Containers and Equipment Categories, with movement history. |
| **Inventory Item** | A tracked stock entry or quantity of club equipment. Belongs to one Equipment Category and one Container, has item attributes, and may be marked as out for maintenance. |
| **Equipment Category** | A classification of Inventory Items, such as swords or masks. Defines the available attributes that can be recorded for items in that category. |
| **Container** | A physical or logical storage location for Inventory Items. Containers may be nested under parent Containers to form a storage hierarchy. |
