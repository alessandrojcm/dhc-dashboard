...
| Term | Definition |
|------|-----------|
| **Member** | A person with a club membership. Owns club profile facts such as identity, contact data, and preferred weapon. Currently has a `user_profiles` record linked to Supabase `auth.users`; the target authentication architecture replaces that link with a DHC-owned Phoenix authentication principal. |
...
Supabase-hosted Postgres and Storage remain in place throughout Phase 1. Supabase Auth remains only until the separately specified Phoenix-auth cutover; Phoenix-owned authentication generated by `phx.gen.auth`, with Assent for Discord OAuth, is the accepted target. PostgREST and RLS policies are removed when all data access is proxied through Phoenix.
