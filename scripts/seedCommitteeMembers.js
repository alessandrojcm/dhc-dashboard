...
async function seedUsers(csvPath) {
	...
	for (const record of records) {
		...
		if (authError) {
			console.error(
				`Error creating auth user for ${record.email}:`,
				authError.message,
			);
		...
		if (profileError) {
			console.error(
				`Error creating profile for ${record.email}:`,
		...
		if (roleError) {
			console.error(
				`Error inserting roles for ${record.email}:`,
				roleError.message,
			);
		...
		const { error: memberError } = await supabase
			...
			.insert({
				...
				preferred_weapon: record.preferred_weapon
					.split(",")
		...
		if (memberError) {
			console.error(
				`Error creating member profile for ${record.email}:`,
				memberError.message,
			);
