...
import type { Workshop, WorkshopInsert, WorkshopUpdate, WorkshopFilters } from './types';
...
export class WorkshopService {
	...
	async findMany(filters?: WorkshopFilters): Promise<Workshop[]> {
		...
		return executeWithRLS(this.kysely, { claims: this.session }, async (trx) => {
			...
			if (filters?.startDateFrom) {
				query = query.where('start_date', '>=', filters.startDateFrom.toISOString());
			}
			if (filters?.startDateTo) {
				query = query.where('start_date', '<=', filters.startDateTo.toISOString());
			}
	 ...
	 */
	async _findById(trx: Transaction<KyselyDatabase>, id: string): Promise<Workshop> {
	 ...
	 */
	async _create(trx: Transaction<KyselyDatabase>, input: WorkshopInsert): Promise<Workshop> {
		const workshopData = {
			...input,
