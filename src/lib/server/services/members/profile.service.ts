...
import { MemberService, type MemberUpdateInput, type MemberData } from './member.service';
...
export class ProfileService {
	...
	constructor(
		...
		this.memberService = memberService ?? new MemberService(kysely, session, logger);
	}
	 ...
	 */
	async updateProfile(userId: string, input: MemberUpdateInput): Promise<MemberData> {
		...
		return executeWithRLS(this.kysely, { claims: this.session }, async (trx) => {
			...
			const currentUser = await this.memberService._getCurrentProfile(trx, userId);
			...
			const updatedMember = await this.memberService._update(trx, userId, input);
