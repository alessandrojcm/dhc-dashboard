...
import { MemberService, MemberUpdateSchema, type UpdateMemberDataArgs } from './member.service';
...
const LOCAL_DB_URL = process.env.DATABASE_URL || 'postgresql://postgres:postgres@localhost:54322/postgres';
	...
	describe('Database Integration Tests', () => {
		...
		describe('updateWithArgs - preferred_weapon', () => {
			it('should update preferred_weapon with single value', async () => {
				...
				console.log('Before update - preferred_weapon:', before.preferred_weapon);
				...
				console.log('After update - preferred_weapon:', result.preferred_weapon);
			...
			it('should persist preferred_weapon after update', async () => {
				...
				const member = await service.findById(testUserId);
				console.log('Fetched after update - preferred_weapon:', member.preferred_weapon);
