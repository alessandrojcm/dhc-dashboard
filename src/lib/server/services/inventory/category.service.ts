...
export class CategoryService {
	 ...
	 */
	async _findById(trx: Transaction<KyselyDatabase>, id: string): Promise<InventoryCategory> {
	 ...
	 * Update a category
	 */
	async update(id: string, input: CategoryUpdateInput): Promise<InventoryCategory> {
	...
	async delete(id: string): Promise<void> {
		...
		try {
			await executeWithRLS(this.kysely, { claims: this.session }, async (trx) => {
				await trx.deleteFrom('equipment_categories').where('id', '=', id).execute();
			});
		} catch (error) {
	 ...
	 */
	async getWithItemCount(id: string): Promise<InventoryCategory & { itemCount: number }> {
