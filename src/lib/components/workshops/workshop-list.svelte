...
let _selectedWorkshop: ClubActivityWithInterest | null = $state(null);
let _showCancellationDialog = $state(false);
let _selectedRegistration: { id: string; status: string } | null = $state(null);
const queryClient = useQueryClient();

const _userData = queryClient.getQueryData(["logged_in_user_data"]) as
	| UserData
	| undefined;

function _getStatusColor(status: string = "planned") {
	switch (status) {
		case "planned":
...
function _hasUserInterest(workshop: ClubActivityWithInterest): boolean {
...
function _handleCancelRegistration(workshop: ClubActivityWithInterest) {
