<script lang="ts">
    ...
    const schema =
        mode === "create" ? CreateWorkshopSchema : UpdateWorkshopSchema;
    ...
    const form = superForm(data.form, {
        ...
        onSubmit: ({ formData }) => {
            ...
            formData.set(
                "workshop_end_date",
                dayjs(
                    formData.get("workshop_end_date") as string,
                ).toISOString(),
            );
        },
        onUpdated: ({ form }) => {
    ...
    function updateWorkshopDates(
        ...
        if (typeof date === "string" && op === "start") {
            ...
            $formData.workshop_date = baseDate
                .hour(hour)
                .minute(minute)
                .toDate();
            return;
        ...
        if (typeof date === "string" && op === "end") {
            ...
            $formData.workshop_end_date = baseDate
                .hour(hour)
                .minute(minute)
                .toDate();
            return;
...
<div class="space-y-8">
    {#if $message?.success}
        <Alert variant="default" class="border-green-200 bg-green-50">
            ...
            <AlertDescription class="text-green-800"
                >{$message.success}</AlertDescription
            >
        </Alert>
    {/if}
    ...
    <form
        method="POST"
        use:enhance
        class="space-y-8 bg-white rounded-lg border shadow-sm p-6"
    >
                    ...
                    {#snippet children({ props })}
                        ...
                        <div class="bg-gray-50 rounded-lg p-4">
                            <Calendar25
                                ...
                                {endTime}
                                onDateChange={(d) =>
                                    updateWorkshopDates(d, "date")}
                                onStartTimeChange={(d) =>
                                    updateWorkshopDates(d, "start")}
                                onEndTimeChange={(d) =>
                                    updateWorkshopDates(d, "end")}
                                disabled={!isWorkshopEditable}
                            />
                        </div>
                        <input
                                    ...
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-blue-700">
                                                Send workshop announcements to
                                                the Discord server
                                            </p>
                                        </div>
                                    </div>
                                ...
                                {#snippet children({ props })}
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-blue-700">
                                                Send workshop announcements via
                        ...
                        >
                            ...
                            <div>
                                ...
                                <p class="text-sm text-blue-700 mt-1">
                                    Enable this to allow non-members to register
                                    for the workshop
                                </p>
                            </div>
                        </div>
