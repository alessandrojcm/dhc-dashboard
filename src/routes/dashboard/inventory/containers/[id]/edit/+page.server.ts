...
import { createContainerService, ContainerUpdateSchema } from '$lib/server/services/inventory';
...
export const load: PageServerLoad = async ({ params, locals, platform }) => {
	...
	const availableContainers = await containerService.getAvailableParents(params.id);
...
export const actions: Actions = {
	...
	delete: async ({ params, locals, platform, request }) => {
		...
		try {
			const containerService = createContainerService(platform!, session);
			const containerWithCount = await containerService.getWithItemCount(params.id);
