...
import { getKyselyClient, executeWithRLS, sql } from '$lib/server/kysely';
import type { InventoryAttributes } from '$lib/types';

export const load = async ({ params, locals, parent, platform }: PageServerLoadEvent) => {
	...
	const containerData = await executeWithRLS(db, { claims: session }, async (trx) => {
		return Promise.all([
