...
export const load = async ({ params, locals }: { params: any; locals: App.Locals }) => {
	...
	return {
		form: await superValidate({
			...
			available_attributes: (category.available_attributes as any) || {}
...
export const actions = {
	update: async ({ params, request, locals, platform }: { params: any; request: Request; locals: App.Locals; platform: any }) => {
	...
	delete: async ({ params, locals, platform }: { params: any; locals: App.Locals; platform: any }) => {
		const session = await authorize(locals, INVENTORY_ROLES);
