...
import { adminInviteRemoteSchema, bulkInviteRemoteSchema, bulkInviteSchema } from '$lib/schemas/adminInvite';
 ...
 */
export const validateSingleInvite = form(adminInviteRemoteSchema, async (data) => {
 ...
 */
export const submitBulkInvites = command(bulkInviteRemoteSchema, async (data) => {
	const event = getRequestEvent();
	const { session } = await event.locals.safeGetSession();

	invariant(session === null, 'Unauthorized');
	const roles = getRolesFromSession(session!);
	invariant(roles.intersection(allowedToggleRoles).size === 0, 'Unauthorized', 403);
