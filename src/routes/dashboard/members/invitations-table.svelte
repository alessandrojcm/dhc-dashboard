...
const _hasSelectedRows = $derived(selectedRows.size > 0);
const _selectedRowsArray = $derived(Array.from(selectedRows));
...
function _onSearchChange(newSearch: string) {
	const newParams = new URLSearchParams(page.url.searchParams);
...
const _bulkResendInvitations = createMutation(() => ({
...
const _bulkDeleteInvitations = createMutation(() => ({
	mutationFn: async (selectedIds: string[]) => {
