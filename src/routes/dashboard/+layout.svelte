...
const { children, data }: { data: LayoutData; children: any } = $props();
const supabase = $derived(data.supabase);
const session = $derived(data.session);
const _roles = $derived.by(() => new Set(data.roles));
const paths = $derived.by(() => page.url.pathname.split("/"));
const _userDataQuery = createQuery<UserData>(() => ({
...
function _getLink(item: string): string {
