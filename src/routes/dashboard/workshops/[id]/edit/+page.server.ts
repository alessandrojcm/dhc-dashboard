	...
	default: async ({ request, locals, platform, params }) => {
		...
		try {
			...
			if (!workshopEditable) {
				return message(
					form,
					{ error: "Only planned workshops can be edited" },
					{ status: 400 },
				);
			}

			// Check if pricing changes are allowed
			const pricingEditable = await canEditWorkshopPricing(
				params.id,
				platform!,
			);
			if (
				!pricingEditable &&
				(form.data.price_member !== undefined ||
					form.data.price_non_member !== undefined)
			) {
				return message(
					form,
					{
