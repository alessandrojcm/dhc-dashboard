...
export const POST: RequestHandler = async ({
	...
	try {
		...
		if (!validatedData.success) {
			const errorMessage =
				validatedData.issues[0]?.message || "Invalid request data";
		...
		if (!member?.customer_id) {
			return json(
				{ success: false, error: "Member or customer not found" },
				{ status: 404 },
			);
		}
		...
		const updatedSub = await stripeClient.subscriptions.update(
			membershipSub.id,
			{
				pause_collection: {
					behavior: "void",
		...
		if (updatedSub.pause_collection === null) {
			return json(
				{ success: false, error: "Error pausing subscription" },
				{ status: 500 },
			);
		}
