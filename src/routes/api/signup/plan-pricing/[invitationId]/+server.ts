...
async function getPricingDetails(
	...
	try {
		...
		const discountPercentage =
			totalDiscount > 0
				? Math.round((totalDiscount / initialInvoiceMonthly.subtotal) * 100)
				: 0;
		...
		return {
			...
			discountedMonthlyFee:
				monthlyDiscount > 0 ? nextMonthInvoice.amount_due - monthlyDiscount : 0,
			discountedAnnualFee:
				annualDiscount > 0 ? nextJanuaryInvoice.amount_due - annualDiscount : 0,
...
export const POST: RequestHandler = async ({ request, params, platform }) => {
	try {
		...
		if (
			couponCode.output.code.toLowerCase().trim() ===
