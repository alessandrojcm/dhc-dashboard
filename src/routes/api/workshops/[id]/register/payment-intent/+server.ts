...
export const POST: RequestHandler = async ({
	...
	try {
		...
		const existingRegistration = await executeWithRLS(
			kysely,
			{ claims: session },
			async (trx) =>
				trx
		...
		// Check capacity
		const registrationCount = await executeWithRLS(
			kysely,
			{ claims: session },
			async (trx) =>
				trx
		...
		const paymentIntentData: Parameters<
			typeof stripeClient.paymentIntents.create
		>[0] = {
			amount,
			currency,
			metadata: {
		...
		const paymentIntent =
			await stripeClient.paymentIntents.create(paymentIntentData);
