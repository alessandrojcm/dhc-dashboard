...
export function createWorkshopService(
	...
	return new WorkshopService(getKyselyClient(platform.env.HYPERDRIVE), session, logger);
}
```
...
export class WorkshopService {
	...
	async _create(trx: Transaction<KyselyDatabase>, input: WorkshopCreateInput): Promise<Workshop> {
		return trx.insertInto('club_activities').values(input).returningAll().executeTakeFirstOrThrow();
	}
}
```
