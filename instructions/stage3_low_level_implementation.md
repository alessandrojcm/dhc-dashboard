...
export const POST: RequestHandler = async ({ request, params, locals }) => {
	try {
		...
		const registrationId = await executeWithRLS(kysely, { claims: session }, async (trx) =>
			trx
				.selectFrom(
					trx
						.fn('register_for_workshop', [
							workshopId,
							memberUserId,
...
{#if registrationStep === 'form'}
	<form method="POST" use:enhance class="space-y-4">
		...
		<div>
			...
			<Input id="phoneNumber" name="phoneNumber" bind:value={$form.phoneNumber} />
		</div>
		...
		<div class="flex items-center space-x-2">
			...
			/>
			<Label for="confirmTerms" class="text-sm">I agree to the terms and conditions</Label>
		</div>
